...
internal class UpdateStateHealthCategoryCommandHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<UpdateStateHealthCategoryCommand, MessageResponse>
{
    private readonly IGenericRepository<HealthCategory> _healthCategoryRepository = unitOfWork.Repository<HealthCategory>();
    public async Task<MessageResponse> Handle(UpdateStateHealthCategoryCommand request, CancellationToken cancellationToken)
    {
